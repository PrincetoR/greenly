  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="presentation">
      ...
      <div role="dialog" aria-modal="true" aria-labelledby="welcome-popup-title" className="relative w-full overflow-hidden rounded-card bg-surface shadow-xl" style={{ maxWidth: `min(${popup.width}px, calc(100vw - 32px))` }}>
        ...
        {(popup.title || popup.body || popup.href) && (
          <div className="flex flex-col gap-3 p-5 sm:p-6">
            ...
            {popup.href && (
              <div className="flex flex-wrap gap-2 pt-1">
                {popup.href.startsWith('/') ? (
                  <Link href={popup.href} onClick={close} className={buttonStyles({ size: 'lg' })}>
                    {popup.buttonLabel || 'ดูรายละเอียด'}
                  </Link>
                ) : (
                  <a href={popup.href} target="_blank" rel="noreferrer" onClick={close} className={buttonStyles({ size: 'lg' })}>
                    {popup.buttonLabel || 'ดูรายละเอียด'}
                  </a>
                )}
                <button type="button" onClick={close} className={buttonStyles({ size: 'lg', variant: 'secondary' })}>
                  ปิด
                </button>
              </div>
            )}
        ...
        {!popup.title && !popup.body && !popup.href && popup.image && <span className="sr-only" id="welcome-popup-title">ประกาศ</span>}
      </div>
    </div>
  );
}
