...
import { UPLOADED_IMAGE, bahtInput, checkbox, intInput, slugInput } from './common';
...
export const categorySchema = z.object({
  ...
  image: z.preprocess((v) => (typeof v === 'string' && v.trim() ? v.trim() : null), z.string().regex(UPLOADED_IMAGE, 'รูปไม่ถูกต้อง').max(300).nullable()),
...
export const productSchema = z.object({
  ...
  images: z.array(z.string().regex(UPLOADED_IMAGE, 'รูปไม่ถูกต้อง')).max(6, 'ใส่รูปได้ไม่เกิน 6 รูป'),
  active: checkbox,
  featured: checkbox,
});
