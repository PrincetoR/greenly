...
- **แถวเลื่อนข้างหน้าแรก** (`components/shop/h-scroller.tsx` client: snap-x, จุดบอกหน้า, ขอบจาง, ปุ่มซ้ายขวาบนจอมีเมาส์, `scroll-pl` = padding ไม่งั้นการ์ดแรกโดนดึงใต้ padding) + `lib/grid-cell.ts` `gridCell(i,{m,s,l})` วาง grid-row/column ต่อ breakpoint ผ่าน CSS vars (`.hs-cell` ใน globals) ให้เรียง**ซ้าย→ขวาแล้วลงแถวล่าง**ทีละหน้า (grid-flow-col เรียงลงคอลัมน์ก่อน ไม่ตรงที่พี่ต่อต้องการ) · **ห้ามใส่ grid-rows-2** — แถว 2 เกิดเองเมื่อมีของ (แถวเดียวไม่เว้นที่) · หมวดหมู่: มือถือ 3/แถว · sm 4 · lg 8 · 2 แถว/หน้า · โปรโมชัน: มือถือ 1/แถว · sm+ 2/แถว · 2 แถว/หน้า · seed 8 หมวด 30 สินค้า 8 โปร (มี MATCHA50 คูปองลดบาทเฉพาะสินค้า p-002)
