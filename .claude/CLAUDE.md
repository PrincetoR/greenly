...
- หน้าแรกด้านบน = **สไลด์แบนเนอร์** จากหลังบ้าน — **อยู่ในคอนเทนเนอร์** (`max-w-6xl px-4 pt-4` ขอบตรงการ์ด 80–1200) สัดส่วน 8:3 (มือถือ 16:9) มุมมน ขอบบาง · ข้อความ**มุมล่างซ้าย** ไล่เฉดจากล่าง · จุดมุมล่างขวา (พี่ต่อ: เต็มจอใหญ่ไป ข้อความกลางไม่เอา) (ไม่มีสไลด์ = ชื่อร้าน+สโลแกนแบบเดิม) · h1 ชื่อร้าน `sr-only` (e2e/07 นับ h1) · หัวข้อสไลด์เป็น `<p>` ไม่ใช่ h2 (e2e นับ `main h2` เป็นลำดับ section) · **ป๊อปอัปตอนเข้าเว็บ** เปิดเฉพาะ seed สาธิต (`enabled: WITH_HISTORY`) เพราะบังปุ่มที่ e2e กด · version เปลี่ยนทุกครั้งที่บันทึก → ลูกค้าที่ปิดแล้วเห็นใหม่
