...
- การ์ดโปรโมชัน = `promo-card.tsx` (server เตรียม `PromoCardData`: ประโยคสรุป/สิทธิ์เหลือ/สินค้า-หมวดในโปร/เงื่อนไข) → `promo-card-view.tsx` (client): **ชื่อ 1 บรรทัด truncate · บรรทัด 2 สถานะ · คำอธิบาย line-clamp-2** · กดทั้งการ์ด (role=button) เปิด **ป๊อปอัปรายละเอียดเต็ม** (ช่วงเวลา นับถอยหลัง สิทธิ์ เงื่อนไข โค้ด+ปุ่มคัดลอก สินค้า/หมวดในโปร ปุ่มดูสินค้า/ปิด Esc/คลิกนอก) — พี่ต่อสั่ง 2026-09-15 · **ไม่มีกล่อง "ใช้โค้ด" บนการ์ด** (ย้ายเข้าป๊อปอัป) · article ต้องมี `min-w-0` (grid track ถ่างจากชื่อ nowrap) · e2e จับการ์ดสินค้าด้วย `.group:has(h3)` (สไลด์หน้าแรกก็เป็น `.group`)
