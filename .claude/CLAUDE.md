...
- **แถบสถานะบนสุด (แบบ Shopee, md+, ไม่ sticky)**: ซ้าย tagline · ขวา รายการโปรด (นับ) · ประวัติการสั่งซื้อ · ชื่อ login (username) หรือ `guest` → /account · พื้นขาวต่อเนื่องกับ header ไม่มีเส้นคั่น ลิงก์เทา ชื่อเข้ม · ต้องเป็น snap point (`.status-bar`) ไม่งั้นโหลดมาโดน snap เลื่อนพ้นจอ
- โลโก้ = ชื่อร้านตัวหนา 30px ไม่มีไอคอน (พี่ต่อเอาใบไม้ออก) สองโทน: 2 ตัวท้ายเป็น `--logo-accent` ชมพู (#e0457b) · ช่องโลโก้กว้างได้ ~112px ถ้าชื่อร้านยาวจะโดน truncate
