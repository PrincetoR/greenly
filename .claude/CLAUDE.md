...
- header ลูกค้ามุมขวา: **[บัญชี ▾] [ตะกร้า]** — ตะกร้าต้องอยู่ขวาสุดเสมอไม่มีอะไรมากั้น (พี่ต่อ: จุดสามจุดหลังตะกร้า "เหมือนกั้นรถเข็น") · AccountMenu = ไอคอนรูปคนเฉย ๆ (ไม่มีวงกลม/ลูกศร) ขนาดเดียวกับตะกร้า dropdown: โปรไฟล์ `/account` · รายการโปรด · ประวัติการสั่งซื้อ `/orders` · มือถือซ่อนปุ่มบัญชี ย้ายเข้า drawer
...
- หน้ารายการสินค้า: แถวบน = [หัวข้อว่าเปิดอะไร + จำนวน … ช่องค้นหา · เรียงลำดับ] (ListingToolbar client: ค้นหาทันทีตอนพิมพ์ หน่วง 300ms ไม่มีปุ่ม · SortMenu dropdown เอง) · card หมวดหมู่บรรทัดแรก "หมวดหมู่สินค้า" กึ่งกลางระดับแถวนี้ · เส้นคั่นที่ 40px · "ทั้งหมด" ขอบบนตรงขอบบนการ์ดสินค้า · padding บนหน้า 16px
- **sticky ตอนเลื่อน (จอ md+)**: card หมวดหมู่ (grid item เอง) + wrapper แถว [หัวข้อ ค้นหา เรียงลำดับ] ติดที่ `top-[65px]` (h-16 + border 1) · gap 16 บน/ล่างอยู่ใน wrapper (pt-4/pb-4) ให้เท่าตอนปกติ · พื้นทึบ + ขอบล่างไล่จาง 10px (`.sticky-fade::after`) ให้การ์ดค่อย ๆ หายก่อนถึงขอบ (พี่ต่อไม่เอา backdrop-blur) · แถบขยาย -mx-1 คลุม ring การ์ดที่ล้น 1px · มือถือไม่ sticky
- `sticky` ใน CSS grid ต้องใส่ที่ตัว grid item เอง (ลูกข้างในขยับได้แค่ในช่องที่สูงเท่าเนื้อหา)

## บทเรียนจากการทดสอบ
- ปุ่ม/กล่องที่ต้องสูงเท่า input ให้ใช้ `border` (วาดในกล่อง) ไม่ใช่ `ring` (box-shadow วาดนอกกล่อง → โตกว่า 1px ทุกด้าน พี่ต่อมองออก)
