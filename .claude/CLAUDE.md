...
src/components/ui     Button/Badge/Card/Field/EmptyState/Alert/Table/Switch/ConfirmButton · **Select** (dropdown ของเราเองแทน `<select>` ทุกที่ — ดูใน Convention)
...
- **`TopLine`** (client ใน ShopHeader): เส้น `bg-brand` **3px** (`h-[3px]` — พี่ต่อลอง 2/6/4 แล้วเอา 3) `fixed top-0 z-60` — กลืนกับแถบสถานะตอนอยู่บนสุด เลื่อนลงแล้วเหลือเส้นเขียวคาดบน header ทับขอบบน header 3px (พี่ต่อสั่ง) · ตอนเปลี่ยนหน้า `.top-line-sweep` (แถบขาววิ่ง keyframes ใน globals.css) จับคลิก `a[href]` ภายในที่ document (**ห้ามเช็ค defaultPrevented** เพราะ next/link preventDefault เอง) หยุดเมื่อ `usePathname` เปลี่ยน · query เปลี่ยนอย่างเดียวโชว์ 600ms · guard 8s
...
- หน้าแรกด้านบน = **แบนเนอร์แบบ Shopee** (พี่ต่อขอ): **แถบพื้นขาวสุดจอ** `bg-surface border-b` ต่อจากเส้นล่างของ header (ไม่มีเส้นบนเอง — พี่ต่อสั่ง) ข้างในคอนเทนเนอร์ `max-w-6xl px-4 pb-6` (เนื้อหา 80–1200 · **ไม่เว้นบน** รูปชิดเส้น header · ล่าง 24 — พี่ต่อไล่ปรับ 16/32 → 0/6 → 0/24) grid `2fr_1fr` — **ความสูงแถวมาจากภาพเล็ก `aspect-[2/1]`** (พอดีรูป ไม่ครอปหัว/ท้าย — พี่ต่อเห็นหัวรูปจม 2px ตอนใช้ความสูงคงที่) สไลด์ยืดเท่าแถวแล้วครอปข้าง · **`HeroHeaderSync`** (client): `html.hero-at-top` ตราบใดที่ขอบล่างของ `[data-hero-band]` ยังอยู่ใต้ header → CSS ทำเส้นล่าง `.site-header` โปร่ง (ยังกิน 1px) · **เส้น header รับช่วงเมื่อเส้นล่างของแถบเลื่อนขึ้นมาถึง** = "เส้นเดียวกันหดขึ้นมา" (พี่ต่อย้ำ 2026-09-15 หลังลองแบบโผล่ตอนจางหมดแล้วบอกเร็วไป · ที่ตำแหน่ง snap หมวดหมู่ เส้นแถบอยู่ที่ 41 ใต้ header เส้น header จึงแสดง — e2e/14 ตรวจ) · **จาง** `[data-hero-content]` opacity = 1 − scrollY/(0.3×สูงแถบ) → หายหมดที่ 30% (พี่ต่อสั่ง) · **`SnapWheel`** (client, md+ & pointer:fine): **เลื่อนตามมืออิสระ ไม่ preventDefault** — ฟัง `scrollend` (fallback หน่วง 150ms) แล้ว "เกลี่ย" ไปหัวกลุ่มตามทิศ: ลง = จุดแรก ≥ y−24 · ขึ้น = จุดสุดท้าย ≤ y+24 (จุด = บนสุด · หัวกลุ่ม−81 · ล่างสุด) · ห่างเกินหนึ่งจอไม่บังคับ · **เริ่มเกลี่ยตั้งแต่ช่วงท้ายของแรงเฉื่อย** (พี่ต่อ: หยุดแล้วค่อย snap = กระตุก): wheel listener `passive:false` นับ |deltaY| เล็กลงติดกัน ≥3 และ <10 หลังเคย ≥20 → preventDefault + settle ทันที · ระหว่างเกลี่ย wheel ที่ ≤ lastDelta×1.5+4 (แรงเฉื่อยเดิม) ถูก preventDefault · ใหญ่กว่านั้น = มือขยับใหม่ → ยกเลิก · ล้อเมาส์ทีละติ๊กยังใช้ GESTURE_GAP 90ms · **แอนิเมชันเกลี่ยเขียนเอง** (ไม่ใช้ scrollTo smooth ที่เริ่มจากนิ่งแล้วเด้ง): วัดความเร็วจาก scroll samples 120ms · มีความเร็วไปทางจุดหมาย → Hermite p'(0)=v, p'(1)=0 ระยะเวลา 2d/v (280–900ms) ชะลอสม่ำเสมอจนหยุดพอดี · เริ่มจากนิ่ง → ease-out ออกตัวทันที · wheel ระหว่างเกลี่ย = ยกเลิกให้มือคุม · `data-no-settle` ที่ html ปิดชั่วคราว (e2e) (พี่ต่อ: ขอต่อเนื่องจากความเร็วที่เลื่อน ไม่เด้ง) · ใส่ `html.wheel-snap` เมื่อ `(pointer: fine)` → CSS ปิด `scroll-snap-type` ของเบราว์เซอร์ (สองระบบแย่งกันแล้วกระตุก) จอสัมผัสยังใช้ CSS snap · **header เป็นพื้นทึบ `bg-surface` ไม่ใช้ backdrop-blur** (เบลอแบนเนอร์ที่มุดใต้ header ทุกเฟรม = กระตุก) · `[data-hero-content]` มี `will-change-[opacity]` — เพราะ snap ของเบราว์เซอร์เลือกจุดใกล้สุด หมุนน้อยเด้งกลับ (พี่ต่อ: ต้องหมุนเยอะไป) · ถอดทั้งหมดตอนออกจากหน้า — เลือกวิธีนี้แทนย้ายสไลด์เข้า header เพราะไม่กระทบ sticky/ระยะ 65 ของทุกหน้า — ซ้าย = สไลด์ใหญ่ (`slot: 'main'` เลื่อนเอง ข้อความมุมล่างซ้าย จุดกึ่งกลางล่าง) · **ไม่มีปุ่มในสไลด์/ป๊อปอัป** (พี่ต่อสั่ง) คลิกทั้งภาพ/เนื้อหาไป href · ป๊อปอัปปิดด้วยกากบาท/คลิกนอก/Esc เท่านั้น ขวา = ภาพเล็ก 2 ช่องซ้อนแนวตั้ง (`slot: 'side'` 2 ใบแรกตามลำดับ นิ่ง คลิกได้) · มือถือ: สไลด์ 16:9 + ภาพเล็ก 2 คอลัมน์ข้างล่าง · ไม่มีภาพเล็ก = สไลด์เต็มการ์ด 8:3 · รูปแนะนำ 2:1 ทั้งคู่ (ไม่มีสไลด์ = ชื่อร้าน+สโลแกนแบบเดิม) · h1 ชื่อร้าน `sr-only` (e2e/07 นับ h1) · หัวข้อสไลด์เป็น `<p>` ไม่ใช่ h2 (e2e นับ `main h2` เป็นลำดับ section) · **ป๊อปอัปตอนเข้าเว็บ** เปิดเฉพาะ seed สาธิต (`enabled: WITH_HISTORY`) เพราะบังปุ่มที่ e2e กด · version เปลี่ยนทุกครั้งที่บันทึก → ลูกค้าที่ปิดแล้วเห็นใหม่
- **แถวเลื่อนข้างหน้าแรก** (`components/shop/h-scroller.tsx` client: snap-x, จุดบอกหน้า, ปุ่มซ้ายขวาบนจอมีเมาส์, **ไม่มีขอบไล่จาง** (พี่ต่อไม่เอา), `scroll-pl` = padding ไม่งั้นการ์ดแรกโดนดึงใต้ padding) + `lib/grid-cell.ts`: `gridCell(i,{m,s,l})` วาง grid-row/column ต่อ breakpoint ผ่าน CSS vars (`.hs-cell` ใน globals) ให้เรียง**ซ้าย→ขวาแล้วลงแถวล่าง**ทีละหน้า · `pageSnapClass` snap เฉพาะช่องแรกของหน้า · `pageFillers` ช่องว่างเติมหน้าสุดท้ายให้เต็ม (ซ่อนต่อ breakpoint) — **เลื่อนทีละหน้าเต็ม** (พี่ต่อ: เลื่อนไม่สุด/ค้างครึ่งหน้า) · **scroll container ต้องเป็น grid เอง** (`gridClassName` prop) เพราะ Chrome ไม่นับ padding ท้ายของ block scroller → หน้าสุดท้ายขาด 16px · **ห้ามใส่ grid-rows-2** — แถว 2 เกิดเองเมื่อมีของ (แถวเดียวไม่เว้นที่) · หมวดหมู่: มือถือ 3/แถว · sm 4 · lg 8 · 2 แถว/หน้า · โปรโมชัน: มือถือ 1/แถว · sm+ 2/แถว · 2 แถว/หน้า · seed 8 หมวด 30 สินค้า 8 โปร (มี MATCHA50 คูปองลดบาทเฉพาะสินค้า p-002)
...
- **ห้ามใช้ `<select>` ของเบราว์เซอร์** (พี่ต่อ 2026-09-15: เมนู system ไม่สวย) → `components/ui/select.tsx` `Select` (client) หน้าตาเดียวกับเมนูเรียงลำดับ: ปุ่ม `role=combobox` ขอบ line · เมนู `role=listbox` การ์ดขาว+เงา ติ๊กถูก · **portal ที่ body แบบ fixed** (ไม่โดน overflow การ์ด/ตารางตัด · พลิกขึ้นเมื่อที่ล่างไม่พอ · reposition ตอน scroll/resize) · คีย์บอร์ดครบ (↑↓ Enter Esc Home/End พิมพ์ตัวแรก) · props: `options {value,label,disabled}` · `name` → hidden input ส่งกับฟอร์ม (uncontrolled `defaultValue` / controlled `value`+`onChange(value)`) · `size` md (สูงเท่า input ในฟอร์ม) / sm (h-9 ตารางคิว) / bar (h-10 text-sm แถบเครื่องมือ) · `fit` จองกว้างเท่าข้อความยาวสุด · `align` · `autoSubmit` (แทน AutoSubmitSelect ที่ลบไป) · `placeholder` · ค่าเริ่มต้น `w-full` ถ้าไม่ส่ง className (ส่ง `shrink-0` ให้กว้างตามเนื้อหา) · ไม่มี HTML `required` — พึ่ง zod ฝั่ง server + `aria-invalid` · e2e ใช้ `pick(page, selector, label)` ใน lib.js แทน selectOption
- หน้ารายการสินค้ามือถือ: หัวข้อ → **dropdown หมวดหมู่ (เต็มแถว ใต้หัวข้อ)** → [ค้นหา · เรียงลำดับ] (พี่ต่อสั่ง 2026-09-15) · placeholder ค้นหา = "ค้นหาสินค้า" · เรียงลำดับ 3 ตัวเลือก (ตัด "ชื่อ ก–ฮ" ออก — db ยังรับ sort 'name')
...
- **scroll snap หน้าแรก**: md+ จอสัมผัส = mandatory · **มือถือ (<md) = `proximity`** (กลุ่มสูงกว่าจอ mandatory จะดูกลางกลุ่มไม่ได้ — พี่ต่อถามว่ามือถือไม่ snap หรือ 2026-09-15) · จอมีเมาส์ = JS SnapWheel · `.snap-sections` ที่ root หน้าแรก · ทุก `<Section>` มี `.snap-section` (`scroll-margin-top: 81` = header 65 + 16) · footer snap end · แถบสถานะ snap start — กฎเดียวกับหน้ารายการ (e2e/02 ตรวจ)
