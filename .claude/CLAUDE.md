...
- **`TopLine`** (client ใน ShopHeader): เส้น `bg-brand` 2px `fixed top-0 z-60` — กลืนกับแถบสถานะตอนอยู่บนสุด เลื่อนลงแล้วเหลือเส้นเขียวคาดบน header (พี่ต่อสั่ง) · ตอนเปลี่ยนหน้า `.top-line-sweep` (แถบขาววิ่ง keyframes ใน globals.css) จับคลิก `a[href]` ภายในที่ document (**ห้ามเช็ค defaultPrevented** เพราะ next/link preventDefault เอง) หยุดเมื่อ `usePathname` เปลี่ยน · query เปลี่ยนอย่างเดียวโชว์ 600ms · guard 8s
