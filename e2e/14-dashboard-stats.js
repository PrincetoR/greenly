...
(async () => {
  ...
  await page.evaluate((y) => window.scrollTo(0, y), Math.round(bandH * 0.25));
  await page.waitForTimeout(150);
  ok(Math.abs((await fade()) - 0.5) < 0.05 && (await hb()) === 'rgba(0, 0, 0, 0)', `เลื่อน 25% ของแถบ: จาง 50% · เส้น header ยังไม่โผล่ (opacity ${await fade()})`);
