...
(async () => {
  ...
  ok(JSON.stringify(labels.map((t) => t.trim())) === JSON.stringify(['แดชบอร์ด', 'สินค้า', 'หมวดหมู่', 'โปรโมชัน', 'บทความ', 'หน้าแรก', 'คำสั่งซื้อ', 'จัดส่ง', 'การชำระเงิน', 'ผู้ใช้', 'ตั้งค่าร้าน']), `เมนู: ${labels.join(' · ')}`);
  ...
  ok(JSON.stringify(staffLabels.map((t) => t.trim())) === JSON.stringify(['แดชบอร์ด', 'สินค้า', 'หมวดหมู่', 'บทความ', 'คำสั่งซื้อ', 'จัดส่ง']), `staff: ${staffLabels.join(' · ')}`);
